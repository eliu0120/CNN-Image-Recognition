...
def batchnorm_forward(x, gamma, beta, bn_param):
  ...
  if mode == 'train':
    ...
    cache = (x_hat, gamma, eps, x, sample_mean, sample_var)
...
def batchnorm_backward(dout, cache):
  ...
  x_hat, gamma, eps, x, sample_mean, sample_var = cache
  ...
  dmu = - np.sum(dx_hat, axis = 0) / b - 2 / N * np.sum(a, axis = 0) * dvar

  dx = da + 2 * a / N * dvar + dmu / N
